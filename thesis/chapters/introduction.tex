...
\begin{itemize}
    ...
    \item \textbf{ APS Dual-Layer LLM Defense : }We developed a dual-layer defense mechanism to enhance the safety and robustness of large language models against malicious prompts. The first layer employs the Detoxify model (Toxic-BERT), a transformer-based classifier, to detect and mitigate toxic content in input prompts, blocking those with a toxicity score $\geq 0.6$. Prompts passing this check proceed to the second layer, where a DistilBERT classifier, fine-tuned with LoRA (Low-Rank Adaptation) on a diverse dataset of 3,000 labeled prompts, identifies adversarial jailbreak attempts. This dataset, curated from Dolly, OpenOrca, JailbreakHub, and JailbreakBench, ensures comprehensive coverage of varied jailbreak styles and safe prompts. The fine-tuned DistilBERT classifier achieves high accuracy in detecting jailbreak prompts with a probability threshold of $\geq 0.6$, demonstrating excellent performance across multiple datasets. The mechanism maintains a low false positive rate (FPR), ensuring minimal erroneous blocking of safe prompts. This well-structured dual-layer defense significantly strengthens model resilience against toxic and adversarial inputs, enabling safe and reliable response generation.
    \item \textbf{Implementation of an Adaptive ASR Defense Pipeline: }For automatic speech
    recognition, we developed an adaptive defense system combining lightweight detection
    using 8-bit quantization with targeted mitigation through Spectral Gating and MP3
    compression, achieving 99% accuracy in adversarial detection while reducing processing
 time by 25% compared to static defenses.
