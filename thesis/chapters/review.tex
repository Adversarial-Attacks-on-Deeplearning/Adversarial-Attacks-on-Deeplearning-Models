...
LLMs introduce a new paradigm of attack that targets their reasoning and instruction-following capabilities.
\begin{itemize}
    \item \textbf{Jailbreaking:} This aims to bypass safety alignments. White-box methods like \textbf{GCG} optimize an "adversarial suffix" to append to a malicious prompt \cite{zou2023universaltransferableadversarialattacks}. Black-box methods use techniques like automated prompt rewriting with another LLM (\textbf{PAIR} \cite{chao2025jailbreaking}) or game-theoretic red-teaming \cite{ma2024evolvingdiverseredteamlanguage}.
    \item \textbf{Prompt Injection:} This attack hijacks the model's behavior by embedding hidden commands in its input. \textbf{Indirect Prompt Injection (IPI)} is particularly dangerous, as the malicious instruction can be hidden in external data that the LLM processes, enabling remote and autonomous exploitation \cite{greshake2023youvesignedforcompromising}.
\end{itemize}
